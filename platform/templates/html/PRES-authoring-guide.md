...
**2026-09-11 eacct_chatbot 사례 — "직전 파일 복사" 관성 문제**: 신규 보고서 2건(`현재 기능 강점 및 고도화 방향`, `경쟁력 비교 및 솔루션화 검토`)을 이 원본 템플릿이 아니라 프로젝트 폴더의 기존 보고서 파일(`신규 도입 배경 및 MCP 적용 보고서_20260702.html`, 2026-07-03 애플룩 전환 이전의 dark-navy 레거시 스타일)을 복사해 작성했다. 원본 템플릿을 Read하지 않았기 때문에 색상 테마 자체가 이미 현재 표준(Apple-look)에서 벗어나 있었고, 그 사본 안에서 `code{font-family:...}` 규칙이 누락돼 있던 것까지 그대로 복제됐다. 이후 대응으로 프로젝트 폴더 안에 별도 규칙 문서(`_manage/reports/TEMPLATE_RULES.md`)를 새로 만들었으나, 이는 SoT를 이 가이드에서 프로젝트 로컬 문서로 이중화하는 잘못된 조치였으므로 삭제하고 이 문서로 병합했다(Jacey 지시). **재발방지**: PRES 문서를 새로 만들 때는 절대 프로젝트 폴더의 기존 HTML을 복사하지 않는다 — 매번 `platform/templates/html/PRES_presentation_template.html` 원본을 Read해서 그 내용을 그대로 복사하는 것부터 시작한다(§0 위 원칙과 동일, 실제 발생 사례로 재확인). 프로젝트가 여러 세션에 걸쳐 진행되면 "직전에 만든 파일을 복사"하는 관성이 생기기 쉬우므로 매번 원본 경로를 의식적으로 확인한다.

> **참고 — eacct_chatbot 프로젝트의 기존 보고서 레거시 상태**: `projects/eacct_chatbot/_manage/reports/`의 기존 파일 대부분(`신규 도입 배경...`, `비용 예측...` 등)이 2026-07-03 애플룩 전환 **이전** 스타일(dark-navy hero, `--navy-950`/`--sky-400:#38BDF8` 계열)로 만들어져 있어, 지금의 원본 템플릿(Apple-look, `--navy-800:#1D1D1F`/`--sky-400:#0071E3` 계열)과 이미 다르다. 이 폴더에서 새 문서를 작성할 계획이라면, 기존 파일을 참고하지 말고 원본 템플릿부터 다시 시작할 것 — 필요 시 기존 문서들도 현재 템플릿으로 재작성할지 별도로 확인한다.
...
**2026-09-02 `<pre>`/`<code>` 스타일 신규 추가 + `.proof p` 고정폭 제거.** D02 gateway blind-test 보고서 작성 중 두 가지 문제가 발견됐다: (1) 템플릿에 `<pre>`/`<code>`에 대한 CSS가 전혀 없어, diff 코드 블록을 넣을 때 브라우저 기본 monospace 스타일이 그대로 노출되어 본문 폰트(Noto Sans KR)와 이질감이 컸다. (2) `.proof p{max-width:720px}`가 2026-08-20 `.wrap` 확장(`920px` 고정 → `min(--content-w,94vw)`, 기본 1320px) 당시 함께 갱신되지 않아, proof 박스 자체는 넓어졌는데 안의 문단만 옛 720px 기준에 묶여 우측에 큰 여백을 남기고 일찍 줄바꿈되는 문제가 있었다. `code`/`pre`/`pre code` 선택자를 SFMono/Consolas 계열 폰트·배경(`#F0F0F2`/`#F5F5F7`)·패딩으로 신규 추가했다. `.proof p`의 `max-width`는 최초 `100%`로 변경했으나, `.proof::after`(우상단 장식 원, `right:-100px`·`width:300px`)가 차지하는 영역까지 텍스트가 흘러가 원과 겹치는 회귀가 발생해 `calc(100% - 160px)`로 재수정했다 — 박스 우측 원 앞에서 줄바꿈되도록 원의 실제 침범 폭만큼 여백을 고정한 값이다. 두 항목 모두 §0 "허용되는 유일한 예외"(커스텀 배지, `--content-w`) 밖의 값이지만, 신규 스타일 추가가 아니라 누락·stale 값에 대한 결함 수정으로 판단해 반영했다.
