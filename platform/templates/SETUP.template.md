...
| 언어 | Python |
| 협업 방식 | 사용자 + {{ASSISTANT_KR}} (통합 비서) |

### AI 비서 역할

| 비서 | 담당 영역 |
|---|---|
| **{{ASSISTANT_KR}} ({{ASSISTANT_EN}})** | project-hub 통합 비서 — 플랫폼 관리(프로젝트 생성·상태 전환·전역 파일·세션 프로토콜) + 프로젝트 내부 협업(기능 개발·산출물·이슈·히스토리·운영) |
...
├── CLAUDE.md                    ← project-hub 전역 설정 ({{ASSISTANT_KR}})
...
│   ├── TRIGGERS.md              ← {{ASSISTANT_KR}}와 약속된 트리거 목록
│   ├── config\
...
├── projects\                    ← 모든 프로젝트
│   └── {프로젝트명}\
│       ├── CLAUDE.md            ← 프로젝트별 {{ASSISTANT_KR}} 설정
│       ├── docs\                ← 산출물 (HTML, 로컬 전용)
│       ├── refs\                ← 참고자료 (로컬 전용)
│       ├── source\              ← 개발 프로젝트만 생성
│       │   ├── src\
...
- `platform/guides/SETUP.md` 자동 생성 (이 파일)

> **기본값 유지:** 이름을 변경하지 않으려면 각 항목에서 Enter만 누르면 됩니다.

> **재실행:** upstream 업데이트 후 `platform/templates/CLAUDE_global.template.md` 또는 `platform/templates/SETUP.template.md`가 변경된 경우
> `python platform/hub_init.py`를 재실행하여 개인화 파일을 재생성하세요.
...
| 파일 | 위치 | 역할 |
|---|---|---|
| 전역 CLAUDE.md | `~\.claude\CLAUDE.md` | {{ASSISTANT_KR}} 전역 설정 (모든 프로젝트에 적용) — hub_init.py로 자동 생성 |
| project-hub CLAUDE.md | `{HUB_ROOT}\CLAUDE.md` | {{ASSISTANT_KR}} 통합 설정 (project-hub 내 적용) |
...
```markdown
## 역할
- {{ASSISTANT_KR}}: project-hub 통합 비서 — 플랫폼 관리 + 프로젝트 내부 협업
```
...
전체 프로젝트 현황. {{ASSISTANT_KR}}가 통합 관리.
...
{{ASSISTANT_KR}}와 약속된 트리거 목록. 새 트리거 추가 시 이 파일에 업데이트.
...
- **{{ASSISTANT_KR}} 제안**: 프로젝트 단계 진입 시 필요한 템플릿 판단 → 사용자 승인 후 `docs/` 폴더에 복사 후 작성
- **사용자 직접 요청**: "~~작성해줘" → {{ASSISTANT_KR}}가 해당 템플릿 복사 후 즉시 작성 시작
...
### 세션 시작 프로토콜 ({{ASSISTANT_KR}} 자동 실행)

VS Code에서 project-hub를 열고 Claude Code 세션을 시작하면 {{ASSISTANT_KR}}가 자동으로:
...
### 커밋·PR에 개인 내부 식별자 기록 금지

커밋 메시지·태그 주석·PR 제목·본문에는 **다른 사람이 그대로 이해할 수 있는 표현만** 쓴다. collab 문서 식별자(`D01`·`DEV_D01~D06`·`D02B`), collab 번들·세션 번호(`20260701-1737`), 개인 관리 문서 ID(`TC-C09`·`T032`·`H-001`·`E-003`)는 기록 금지. 사내에서 조회 가능한 식별자(Jira 이슈 키·Bamboo 빌드 키·릴리스 태그·경로·기능명)만 허용한다. 리모트 종류와 무관하게 전 리모트 적용. 상세 기준 → 라이프사이클 §5-3-1.
...
"오늘 작업 정리해줘" 트리거 시 {{ASSISTANT_KR}}가 자동 판단:
...
| 위치 | 대상 | 관리자 |
|---|---|---|
| `TODO_GLOBAL.md` | 프로젝트 무관 독립 항목 | {{ASSISTANT_KR}} |
| `projects\{이름}\_manage\todo.md` | 해당 프로젝트 항목 | {{ASSISTANT_KR}} |

전체 현황 필요 시: "전체 To-Do 보여줘" → {{ASSISTANT_KR}}가 TODO_GLOBAL + 진행중·활성 프로젝트 todo.md 통합 출력.

### 이슈

| 위치 | 대상 | 관리자 |
|---|---|---|
| `ISSUES_GLOBAL.md` | 프로젝트 무관 독립 항목 | {{ASSISTANT_KR}} |
| `projects\{이름}\_manage\issues.md` | 해당 프로젝트 항목 | {{ASSISTANT_KR}} |

전체 현황 필요 시: "전체 이슈 보여줘" → {{ASSISTANT_KR}}가 ISSUES_GLOBAL + 진행중·활성 프로젝트 issues.md 통합 출력.
