...
> `AGENTS.md`는 공유 root entrypoint로서 COMMON 핵심 9개 block만 인라인으로 포함한다. 나머지 block은 필요 시 `COMMON.md`를 직접 Read한다. 트리거 목록(SoT)은 `platform/TRIGGERS.md` 참조 — "아이다 실수 시인" 등 자동 실행 트리거는 이 entrypoint를 쓰는 하네스에도 동일 적용된다.
