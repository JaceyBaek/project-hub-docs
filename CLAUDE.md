...
2. **불필요한 서두 금지** — 바로 본론
3. **사실 기반만** — 추측 불가 / 제안은 "제안" 명시 / 불확실하면 모른다고. 외부 서비스·API 스펙은 검색 확인 후 기술 — 미확인 수치 산출물·답변 기재 금지. **Confluence 등 외부 위키 페이지 내용을 답변할 때는 로컬에 저장된 인용/캐시 문서(예: 특정 페이지를 출처로 명시한 레포 내 doc)를 "그 위키 페이지의 현재 내용"과 동일시하지 않는다** — 실시간 조회가 가능하면 먼저 조회하고, 불가능하면 "로컬 문서 기준(최신 버전 미확인)"임을 답변에 명시한다. **여러 계층(클라이언트→서버→DB 등)에 걸친 기능을 "완료"로 보고하기 전, 코드 작성 완료와 실제 엔드투엔드 동작 검증 완료를 구분한다** — 계층 중 일부(예: 서버 로직)만 코드 리뷰로 확인하고 나머지 계층(예: 클라이언트 호출부가 실제로 필요한 헤더·파라미터를 보내는지)은 라이브로 확인하지 않은 채 전체를 "완료"로 보고하지 않는다. 작업 계획에 검증 체크리스트가 있으면 그 항목을 실제로 실행(또는 실행 불가 사유 명시)한 뒤에만 완료로 보고한다(2026-08-18 eacct_chatbot `/compact` DB 저장 기능 사례 — server.py·chat_handler.py·context_store.py 저장 로직 구현은 완료했으나 widget.html의 실제 호출부가 `X-Session-ID` 헤더를 보내지 않는 것을 라이브로 확인하지 않고 "완료"로 보고, Jacey가 직접 테스트해서야 저장 실패를 발견).
