...
> 본 문서는 산출물 템플릿(`platform/templates/deliverables/*.html`, `platform/templates/deliverables/guides/*.md`, `platform/templates/deliverables/assets/`, `platform/templates/deliverables/index.html`)을 사내 환경에 배포하는 표준 절차를 정의한다.
>
> 모든 경로는 저장소 루트(`gsretail-ax/project-hub`) 기준 상대 경로 `platform/templates/deliverables/` 하위로 통일한다.
...
- 저장소: **`gsretail-ax/project-hub`**
- 자산 위치: `platform/templates/deliverables/` 하위
...
- 가이드(`platform/templates/deliverables/guides/`)는 **게시 금지**
...
1. `gsretail-ax/project-hub` 저장소에 `platform/templates/deliverables/` 디렉토리로 전체 자산 commit
2. Confluence Space `DOCS-TMPL` 생성
3. Space 권한 설정 (전사 읽기 / PMO 편집)
4. Space 사이드바 헤더에 `platform/templates/deliverables/assets/wireframe.css` attach
...
| 2 | 로컬에서 `platform/templates/deliverables/index.html` 열어 링크/렌더 확인 | 문서 오너 | 검증 스크린샷 |
| 3 | PR 생성 → **Jacey 단독 검토 및 승인** | 문서 오너 + Jacey | PR merge |
...
- [ ] `platform/templates/deliverables/assets/wireframe.css` 경로가 Space 첨부 경로로 치환됐다
...
## 5. AI 작성 가이드 배포 (`platform/templates/deliverables/guides/`)
...
- 변경 이력은 `platform/templates/deliverables/CHANGELOG.md` 에 누적 (별도 작성 권장)
...
| 용도 | 도구 |
|---|---|
| Git 호스팅 | GitHub (`gsretail-ax/project-hub`) |
| Confluence 자동화 | `platform/scripts/manage/wiki_sync.py` (Atlassian REST API 기반 사내 스크립트) |
...
| 역할 | 담당 | 책임 |
|---|---|---|
| 검토/승인 | **Jacey** | PR 단독 검토·승인, 템플릿 콘텐츠 오너십 |
| Confluence 관리자 | (별도 지정) | Space 권한, 매크로 플러그인, 페이지 게시 |
| AI 운영 | **아이다** | 가이드 컨텍스트 패키징, `wiki_sync.py` 운영, 챗봇 메타데이터 동기화 |
