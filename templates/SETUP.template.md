...
8. [GitHub 저장소 연결](#8-github-저장소-연결)
9. [Git Flow 브랜치 전략](#9-git-flow-브랜치-전략)
10. [Python 개발 환경 설정](#10-python-개발-환경-설정)
11. [코드 품질 도구 설정](#11-코드-품질-도구-설정)
12. [GitHub Actions CI/CD](#12-github-actions-cicd)
13. [알림 설정](#13-알림-설정)

**[Part 3 — 운영 가이드]**
14. [새 프로젝트 시작](#14-새-프로젝트-시작)
15. [개발 워크플로우](#15-개발-워크플로우)
16. [히스토리 관리](#16-히스토리-관리)
17. [To-Do 및 이슈 관리](#17-to-do-및-이슈-관리)
18. [트러블슈팅](#18-트러블슈팅)
19. [부록 — 네이밍 규칙](#19-부록--네이밍-규칙)
...
| 언어 | Python |
| 협업 방식 | 사용자 + {{HUB_ASSISTANT_KR}}(플랫폼) + {{PROJ_ASSISTANT_KR}}(프로젝트) |

### AI 비서 역할

| 비서 | 담당 영역 |
|---|---|
| **{{HUB_ASSISTANT_KR}} ({{HUB_ASSISTANT_EN}})** | project-hub 플랫폼 관리 — 프로젝트 생성·상태 전환·전역 파일 관리·세션 프로토콜 |
| **{{PROJ_ASSISTANT_KR}} ({{PROJ_ASSISTANT_EN}})** | 각 프로젝트 내부 협업 — 기능 개발·산출물 작성·이슈·히스토리·운영 |
...
├── CLAUDE.md                    ← project-hub 전역 설정 ({{HUB_ASSISTANT_KR}}+{{PROJ_ASSISTANT_KR}})
├── TRIGGERS.md                  ← {{HUB_ASSISTANT_KR}}/{{PROJ_ASSISTANT_KR}}와 약속된 트리거 목록
...
│   └── {프로젝트명}\
│       ├── CLAUDE.md            ← 프로젝트별 {{PROJ_ASSISTANT_KR}} 설정
│       ├── docs\                ← 산출물 (HTML, 로컬 전용)
│       ├── refs\                ← 참고자료 (로컬 전용)
...
| [2/3] | 플랫폼 비서 한국어 이름 | project-hub 관리 담당 AI | 세라 |
| [2/3] | 플랫폼 비서 영문 이름 | — | Sera |
| [2/3] | 플랫폼 비서 이름 의미 | — | 섬기는/돕는 자 (라틴어) |
| [3/3] | 프로젝트 비서 한국어 이름 | 프로젝트 내부 작업 담당 AI | 아이다 |
| [3/3] | 프로젝트 비서 영문 이름 | — | Aida |
| [3/3] | 프로젝트 비서 이름 의미 | — | 돕는 자 |

**실행 결과:**
- `config/personal.yml` 저장 (gitignored — 커밋되지 않음)
- `C:\Users\{사용자명}\.claude\CLAUDE.md` 자동 생성 (전역 개인화 설정)
- `guides/SETUP.md` 자동 생성 (이 파일)

> **기본값 유지:** 이름을 변경하지 않으려면 각 항목에서 Enter만 누르면 됩니다.

> **재실행:** upstream 업데이트 후 `templates/CLAUDE_global.template.md` 또는 `templates/SETUP.template.md`가 변경된 경우
> hub_init.py를 재실행하여 개인화 파일을 재생성하세요.
...
| 파일 | 위치 | 역할 |
|---|---|---|
| 전역 CLAUDE.md | `~\.claude\CLAUDE.md` | {{PROJ_ASSISTANT_KR}} 전역 설정 (모든 프로젝트에 적용) — hub_init.py로 자동 생성 |
| project-hub CLAUDE.md | `{HUB_ROOT}\CLAUDE.md` | {{HUB_ASSISTANT_KR}}+{{PROJ_ASSISTANT_KR}} 통합 설정 (project-hub 내 적용) |
...
```markdown
## 역할
- {{HUB_ASSISTANT_KR}}: project-hub 플랫폼 관리 전담
- {{PROJ_ASSISTANT_KR}}: 각 프로젝트 내부 협업 전담
```
...
- **역할 자동 전환 규칙**: 요청 내용으로 {{HUB_ASSISTANT_KR}}/{{PROJ_ASSISTANT_KR}} 자동 판단
...
전체 프로젝트 현황. {{HUB_ASSISTANT_KR}}가 통합 관리.
...
{{HUB_ASSISTANT_KR}}/{{PROJ_ASSISTANT_KR}}와 약속된 트리거 목록. 새 트리거 추가 시 이 파일에 업데이트.
...
- **{{PROJ_ASSISTANT_KR}} 제안**: 프로젝트 단계 진입 시 필요한 템플릿 판단 → 사용자 승인 후 `docs/` 폴더에 복사 후 작성
- **사용자 직접 요청**: "~~작성해줘" → {{PROJ_ASSISTANT_KR}}가 해당 템플릿 복사 후 즉시 작성 시작
...
### 세션 시작 프로토콜 ({{HUB_ASSISTANT_KR}} 자동 실행)

VS Code에서 project-hub를 열고 Claude Code 세션을 시작하면 {{HUB_ASSISTANT_KR}}가 자동으로:
...
마무리 표현 감지 시 {{HUB_ASSISTANT_KR}}가:
...
"오늘 작업 정리해줘" 트리거 시 {{HUB_ASSISTANT_KR}}가 자동 판단:
...
| 위치 | 대상 | 관리자 |
|---|---|---|
| `TODO_GLOBAL.md` | 프로젝트 무관 독립 항목 | {{HUB_ASSISTANT_KR}} |
| `projects\{이름}\_manage\todo.md` | 해당 프로젝트 항목 | {{PROJ_ASSISTANT_KR}} |

전체 현황 필요 시: "전체 To-Do 보여줘" → {{HUB_ASSISTANT_KR}}가 TODO_GLOBAL + 진행중·운영중 프로젝트 todo.md 통합 출력.

### 이슈

| 위치 | 대상 | 관리자 |
|---|---|---|
| `ISSUES_GLOBAL.md` | 프로젝트 무관 독립 항목 | {{HUB_ASSISTANT_KR}} |
| `projects\{이름}\_manage\issues.md` | 해당 프로젝트 항목 | {{PROJ_ASSISTANT_KR}} |

전체 현황 필요 시: "전체 이슈 보여줘" → {{HUB_ASSISTANT_KR}}가 ISSUES_GLOBAL + 진행중·운영중 프로젝트 issues.md 통합 출력.
